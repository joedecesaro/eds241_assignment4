...
huxreg(fs1, statistics = c(N = "nobs", R2 = "r.squared"))
```
...
huxreg(tsls1, statistics = c(N = "nobs", R2 = "r.squared"))
```

\noindent The TSLS estimated price elasticity for demand of sardines using wind_m_s as an instrument for log_price is `r round(tsls1$coefficients[[2]], 2)`.
...
tsls2_hux <- huxreg(tsls2, statistics = c(N = "nobs", R2 = "r.squared"))
```

```{r, echo = FALSE, message = FALSE, warning = FALSE}
restack_across(tsls2_hux, 17)
```

\noindent The TSLS estimated price elasticity for demand of sardines using wind_m_s as an instrument for log_price and including fixed effects for each year, month, and country is `r round(tsls2$coefficients[[2]], 2)`.
